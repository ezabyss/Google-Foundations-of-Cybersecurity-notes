# 📘 Glossary: Network Traffic and Logs Using IDS & SIEM Tools

| Term | Definition |
|-----|-----------|
| **Anomaly-based analysis** | A detection method that identifies abnormal behavior by comparing activity against a baseline. |
...
| **Endpoint** | Any device connected on a network, such as a laptop, server, or mobile device. |
| **Endpoint detection and response (EDR)** | An application that monitors endpoints for malicious activity and enables investigation and response. |
...
| **Intrusion detection systems (IDS)** | An application that monitors system or network activity and alerts on possible intrusions. |
...
| **Signature analysis** | A detection method used to find events of interest based on known patterns. |
...
| **Zero-day** | An exploit that was previously unknown to vendors or defenders. |
